...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT v.*, u.channel_name FROM videos v JOIN users u ON v.user_id = u.id WHERE v.id = ?");
    ...
    <header class="main-header">
        <div class="container header-content">
            ...
            <div class="search-bar">
                <input type="text" placeholder="Искать видео...">
                    ...
                    <div class="video-details-bar">
                        <p class="video-meta-watch">
                            <span class="channel-name-watch">
                                <a href="channel.php?id=<?php echo $video['user_id']; ?>"><?php echo htmlspecialchars($video['channel_name']); ?></a>
                            </span>
                            <span class="views-watch"><?php echo htmlspecialchars($video['views']); ?> просмотров</span>
                            <span class="upload-date-watch"><?php echo date('d.m.Y', strtotime($video['upload_date'])); ?></span>
                        </p>
                    </div>
                                ...
                                foreach ($recommended_videos as $rec_video): ?>
                                    <a href="watch.php?id=<?php echo $rec_video['id']; ?>" class="video-card">
                                        ...
                                        <div class="video-info">
                                            ...
                                            <p class="video-meta">
                                                <span class="channel-name"><?php echo htmlspecialchars($rec_video['channel_name']); ?></span>
                                                <span class="views"><?php echo htmlspecialchars($rec_video['views']); ?> просмотров</span>
                                                <span class="upload-date"><?php echo date('d.m.Y', strtotime($rec_video['upload_date'])); ?></span>
                                            </p>
                                        </div>
                                    </a>
                                <?php endforeach;
